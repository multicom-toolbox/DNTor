...
=cut
 ...
 use strict; # force disciplined use of variables
  if (@ARGV != 3)
    { # @ARGV used in scalar context = number of args
	  ...
	  print("This script tries to calculate the dope score for all models, two input parameters, one is the input folder, and one output folder!\n");
	  ...
	  print("You should execute the perl program like this: perl $PROGRAM_NAME add_modeller dir_input_target_model dir_output\n");
      print("\n********** example******\n");
 ...
 my($addr_modeller) = abs_path($ARGV[0]);
 my($input_dir)=abs_path($ARGV[1]);
...
my(%hash); # this hash table tries to score the model name and the prediction score
    $read_folder=$addr_output;        # this is the output file path
    $read_folder2=$input_dir;                      # this is the folder for each target
	if(!-s $read_folder2)
	{
...
system("pwd");
        ...
        close(PY);
        $pycommand = "$addr_modeller ".$dope_script;
