...
@tem_split = split(/\//,$addr_goap);
my($path_for_goap) = $tem_split[0];
for(my $ii = 1; $ii<@tem_split-1;$ii++)
{
    $path_for_goap.="/".$tem_split[$ii];
}
...
#print "Check succeed, start ...\n";
	...
	$read_folder=$input_dir;

	$write_path=$addr_output.".TMP";                  # temporary file

    $write_goap=$addr_output;         # the path for store the goap score of this target $file
...
########### use goap to process each model ###############
    ...
    foreach $target (@targets)
    {
        ...
        $tem_folder_root=$input_dir."/".$target.".inp";            # temporary file for the goap score ,
