...
print "\n********************** ab initio for validation ************************\n";
 ...
 my($output)=$ARGV[4];
 if(!-s $input_target || !-s $input_seq)
 {
	print "The input file $input_target or $input_seq does not exists, please check!\n";
	exit(1);
 } 
#############################################################
 my($file,$target);
...
#############################################################
	$seq_path=$input_seq; # the fasta sequence input
    $write_folder=$output;
	`mkdir $write_folder`;   # create write folder
    $path_read=$input_target; # the read folder
...
######### create file to save the score #################
     $file_name=$output."/".$target.".model_eval_score";
			 open (File, "&gt;$file_name");
