 ...
 sub check_it($)
 {
  ...
  if (@ARGV < 4)
    { # @ARGV used in scalar context = number of args
	  ...
	  print "0. Dir of all targets !\n";
	  print "1. dir of fasta sequences \n";
	  print "2. address of Qprob.sh in qprob package \n";
...
print "\n********************** ab initio for validation ************************\n";
 ...
 my(@files,@tt);
	 ...
	 $target_output = $tem_output;
         
	 -s $target_output || system("mkdir $target_output");
	 $path_target=$dir_targets;
	 $path_seq = $dir_fasta;
         $path_write= $dir_output;
         ...
         if($one_model == 1)
         {
                 ...
                 if(!-s $path_seq) {print "cannot find $path_seq";next;}
                  $return_val = system("./$addr_feature_pl $path_seq $path_target $target_output one");
	 ...
	 $return_val = system("$addr_feature_pl $path_seq $path_target $target_output");
	 if($return_val!=0)
	 {
		 print "perl $addr_feature_pl $path_seq $path_target $target_output fails!\n";
	 }
     $path_read = $target_output."/"."DeepQA.Qprob_score";
	 $path_write= $dir_output;
	 system("cp $path_read $path_write");
