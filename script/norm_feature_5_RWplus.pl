...
###################################################################
  if (@ARGV != 3)
    { # @ARGV used in scalar context = number of args
	  print("This script tries to normalize the RWplus score, so we can use the normalized dfire score to train NN network.\n");
	  print "perl $PROGRAM_NAME dir_sequence(*.fasta) dir_not_normed_dfire_score(*.RWplus_score) dir_output\n";
	  
	  print "For example:\n\n";
          print "perl $0 ../format_data/original_data/sequences ../calculated_scores/feature_10_RWplus_predictions ../normed_scores/predictions/feature_10_RWplus_predictions_norm\n";
          print "\nperl $0 ../format_data/original_data/sequences ../Validation/1_calculated_scores/feature_10_RWplus_casp11_stage1 ../Validation/2_normed_scores/casp11_stage1/feature_10_RWplus_casp11_stage1_norm\n";
          print "perl $0 ../format_data/original_data/sequences ../Validation/1_calculated_scores/feature_10_RWplus_casp11_stage2 ../Validation/2_normed_scores/casp11_stage2/feature_10_RWplus_casp11_stage2_norm\n";
	  exit(0);
	}
...
if($count>0) {print "$count at the range [$min,$max]\n";}
	$IN->close();
	$OUT->close();
}
