...
###################################################################
  if (@ARGV != 3)
    { # @ARGV used in scalar context = number of args
	  print("This script tries to normalize the goap score, so we can use the normalized goap score to train NN network.\n");
	  print "perl $PROGRAM_NAME dir_sequence(*.fasta) dir_not_normed_goap_score(*.goap_score) dir_output\n";
	  
	  print "For example:\n\n";
          print "perl $0 ../format_data/original_data/sequences ../calculated_scores/feature_7_GOAP_predictions ../normed_scores/predictions/feature_7_GOAP_predictions_norm\n";
          print "\nperl $0 ../format_data/original_data/sequences ../Validation/1_calculated_scores/feature_7_goap_casp11_stage1 ../Validation/2_normed_scores/casp11_stage1/feature_7_goap_casp11_stage1_norm\n";
          print "perl $0 ../format_data/original_data/sequences ../Validation/1_calculated_scores/feature_7_goap_casp11_stage2 ../Validation/2_normed_scores/casp11_stage2/feature_7_goap_casp11_stage2_norm\n";
	  exit(0);
	}
 ...
 print "You have total $TOTAL_OUT outliers\n";
sub norm_score($$$)
{# normalize the score
	my($input,$len,$out)=@_; 
    my($IN,$OUT,$line);
    ...
    while(defined($line=<$IN>))
	{
        ...
        if($tem_split[1] eq "file") {die "check $line at $input\n";}
        if($tem_split[1] < $min)
		{
			$tem_split[1] = $min;
			$count++;
