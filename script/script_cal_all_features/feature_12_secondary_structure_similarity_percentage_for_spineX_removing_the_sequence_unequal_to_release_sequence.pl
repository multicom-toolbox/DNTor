 ...
 sub cal_normalized_similarity($$$$$$); # calculate the secondary structure similarity for two sequences with unequal sequence length, and normalize the similarity score
  if (@ARGV < 7)
    { # @ARGV used in scalar context = number of args
          print "This is special script, for the model with unequal sequence with released sequence, we skip evaluating them, so that the training data will not be biased later!\n\n\n";
	  
	  print"This script process the secondary structure parsed by DSSP and secondary structure predicted by spineX for one target, if you need to process more than one target, you need to call this script several times. Use the percentage of secondary structure similarity as the quality score!\n";
	  print "\n************** Renzhi Cao *******************\n";
	  print "Input:\n";
...
#		$similarity_score = cal_normalized_similarity($addr_LCS,$path_real,$path_parse,$path_LCS,$spi_ss,$dssp_ss);
              $similarity_score = -1;              # remove the model with unequal sequence to the released one!
	}
