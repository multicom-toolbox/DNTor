 ...
 sub convert_dssp_to_three_ss($); # convert a secondary structure of dssp into helix(G,H,I), strand(E,B), and loop(others).
  ...
  if (@ARGV != 2)
    { # @ARGV used in scalar context = number of args
      ...
      print "Attention, I change it a little bit, so it ranges in [0,1]. The total area times mass of that residue are used to be divided.\n";

          print "Revised by 2/19/2014, use 1-score as the final score!\n";

	  print"This script process the secondary structure parsed by DSSP. Use the weighted exposed area(summation of mass multiply exposed area of each residue) as the quality score!\n";
	  print "\n************** Renzhi Cao *******************\n";
	  print "Input:\n";
	  ...
	  print "\nFor example:\n";
          ...
          print "perl $0 ../dssp_parsed_DB ../1_calculated_scores/feature_1_weighted_DB\n";
	  exit(0);
	}
...
print "Processing $path_target ...\n";
    ...
    $path_out = $dir_output."/".$file.".fraction_of_weighted_exposed_area";      # the output file
	...
	foreach $target (@targets)
	{
		 ...
		 $quality = cal_weighted_exposed($path_model1);     
		 if($quality == -1)
