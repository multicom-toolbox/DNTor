...
=cut
 ...
 sub read_dssp($);              # read the solvent accessbility from dssp parsed file
 sub read_sspro_acc($);            # read the solvent accessibility from sspro output file
 ...
 sub cal_normalized_similarity($$$$$$); # calculate the secondary structure similarity for two sequences with unequal sequence length, and normalize the similarity score
  if (@ARGV < 7)
    { # @ARGV used in scalar context = number of args
          print "Revised at 1/5/2015, now we can process one pdb.\n";	  
	  print"This script process the solvent accessbility parsed by DSSP and the one predicted by sspro for one target, if you need to process more than one target, you need to call this script several times. Use the percentage of secondary structure similarity as the quality score!\n";
	  print "\n************** Renzhi Cao *******************\n";
	  print "Input:\n";
	  print "0. Dir of all models (for one target or a target with different pdbs)!\n";
	  print "1. address of fasta sequence \n";
	  print "2. address of LCS \n";
	  print "3. address of dsspcmbi\n";
	  print "4. address of dssp2dataset.pl \n";
	  print "5. address of predict_acc.sh ( this is in the sspro tool)\n";
	  print "6. Dir of output\n";
          print "(7. Optional) one , use this if you have only one pdb for the 0 parameter\n";

	  print "\n**************** Renzhi Cao *****************\n";
	  print "\nFor example:\n";
	  print "perl $PROGRAM_NAME ../data/casp10_stage1_server_prediction_pdb/T0644 /exports/store1/rcrg4/REAL_NATIVE_from_CASP/data_downloaded_from_CASP/data_sequence/casp10_seq/T0644.fasta /exports/store1/rcrg4/tool_I_develop/LCS /exports/store1/tool/dsspcmbi ../Feature_generate_script/dssp2dataset.pl /exports/store1/tool/sspro4/bin/predict_acc.sh ../test/feature_6_test\n";
	  exit(0);
	}
...
############################################################################
 ...
 -s $addr_sspro || die "cannot open input $addr_sspro\n"; 
...
######## use sspro to predict the solvent accessbility for the input sequence ############
 ...
 my($tem_output)=$dir_output."/"."$sequence_name.acc";
 my($check_path) = $tem_output;
 if(-s $check_path)
 {
    print "already processed the output, $addr_seq, skip!\n";
 ...
 $return_val = system("perl $addr_sspro $addr_seq $tem_output");
 if($return_val !=0)
 {
         print "perl $addr_sspro $addr_seq $tem_output fails\n";
 ...
 my($spi_seq,$spi_ss)=read_sspro_acc($acc_output);
...
foreach $target (@targets)
{
 ...
 ######## calculate the solvent access similarity between sspro predicted and dssp parsed #########

my($dssp_aa,$dssp_ss,$similarity_score,$path_LCS,$path_real,$path_parse);
my($LCS_dir)=$dir_output."/"."LCS";
-s $LCS_dir || system("mkdir $LCS_dir");

my($path_ss_similarity)=$dir_output."/".$target_name.".sol_similarity";
my($OUT_SS);
...
foreach $target (@targets)
{
    ...
    ($dssp_aa,$dssp_ss)=read_dssp($path_target);      # get the amino acids and solvent accessibility from the dssp parsed output
	...
	{
		$similarity_score = cal_difference($dssp_ss,$spi_ss);        # calculate the secondary similarity score, no normalization!!!
	}
	else
	{# we need to align the sequence and then normalize the similarity score based on the spineX predicted secondary structure
		...
		$OUT_SS->close();		
		$similarity_score = cal_normalized_similarity($addr_LCS,$path_real,$path_parse,$path_LCS,$spi_ss,$dssp_ss);
	}
